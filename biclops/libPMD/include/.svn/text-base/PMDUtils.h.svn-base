...
public:
    static double GetVersion() { return 9.0f;};

    // OS-independent sleep function (wait is milliseconds)
    static void DoSleep(clock_t wait);
    
    // OS-independent time function. (in milliseconds)
    static double GetTime();

    // Miscellaneous math routines
    static int Crop(int val, int minval, int maxval);
    static int Rnd(double val);
    static bool AlmostEqual(double v1, double v2, double nEpsilon);
...
//    static bool FindToken (FILE *file, char *token, const char *find);
    ...
    static bool LoadConfiguration(const char *fileName, bool recursivelyCalled = false);
    ...
    static bool ConfigureControllers();
    ...
    static bool Initialize(const char *fileName,bool connectToControllers = true,
			   bool configureControllers = true);
    ...
    static AxisVector *CreateAxisLUT(vector<const char*> axisNames);
    static void CreateAxisLUT(vector<const char*> axisNames, AxisVector &axisVector);
    static void GetAxisLUT(vector<const char*> &axisNames, AxisVector &axisVector);
    ...
    static void UpdateAll(AxisList axes);
    static void UpdateAll(AxisVector axes);
    static bool Move(AxisList axes, bool waitForMotionToStop = true);
    static bool Move(AxisVector axes, bool waitForMotionToStop = true);
    static bool Move(Kinematics::DoublesVector &cartesianCoordinate, 
                     AxisVector &axes,
                     Kinematics &kinematics, bool waitForMotionToStop = true);

    // Convert return status from any PMDxxx class method into a textually
    // equivalent human-readable form.
    static const char *GetErrorMessage(PMDuint16 errorCode);

    static std::vector<long double> polyfit(const std::vector<std::vector<double> >&  data_array, int order);
