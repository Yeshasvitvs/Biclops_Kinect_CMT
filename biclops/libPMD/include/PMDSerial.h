...
//    bool HasInterrupt() { return false;};
    ...
    // PMD chipsets prior to Magellan require 2 stop bits when communicating
    // at 57600 or 115200. These methods ensure this.
    // Returns true if any of the parameters were modified.
    bool ConstrainSerialSettings(
    ...
    bool ConstrainBaud(int &baud);

    static const char *StartToken;
    static const char *StopToken;
    static bool TokenToPMDProtocol(char *token, tagPMDSerialProtocol &protocol);
    void ReadConfig(FILE *file, char *token);

    PMDuint16 Sync();

    tagPMDProductFamily prodFamily;
...
private:
    ...
    tagPMDSerialProtocol protocol;
    PMDuint8             multiDropID;
    unsigned int         commandAttemptLimit; // # of tries allowed for a command
                            // A value > 1 allows retries upon comm failure.
