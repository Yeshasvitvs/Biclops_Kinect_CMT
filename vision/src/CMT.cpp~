...
void inout_rect(const std::vector<cv::KeyPoint>& keypoints, cv::Point2f topleft, cv::Point2f bottomright, std::vector<cv::KeyPoint>& in, std::vector<cv::KeyPoint>& out)
{
    for(unsigned int i = 0; i < keypoints.size(); i++)
    {
        if(keypoints[i].pt.x > topleft.x && keypoints[i].pt.y > topleft.y && keypoints[i].pt.x < bottomright.x && keypoints[i].pt.y < bottomright.y)
            in.push_back(keypoints[i]);//Key points inside the bounding box - Selected Key points
        else out.push_back(keypoints[i]);//Key points outside the bounding box - Background Keypoints
    }
}
...
void track(cv::Mat im_prev, cv::Mat im_gray, const std::vector<std::pair<cv::KeyPoint, int> >& keypointsIN, std::vector<std::pair<cv::KeyPoint, int> >& keypointsTracked, std::vector<unsigned char>& status, int THR_FB)
{
    ...
    {
        std::vector<cv::Point2f> pts;//KeyPoints
        std::vector<cv::Point2f> pts_back;//key points used in backward optical flow
        std::vector<cv::Point2f> nextPts;//key points found in forward optical flow 
        ...
        for(unsigned int i = 0; i < keypointsIN.size(); i++)
            pts.push_back(cv::Point2f(keypointsIN[i].first.pt.x,keypointsIN[i].first.pt.y));//Pushing back into keypoints vector
        ...
        for(unsigned int i = 0; i < pts.size(); i++)
        {
            ...
            fb_err.push_back(sqrt(v.dot(v)));//pushing back into forward error vector
        }
...
CMT::CMT()//Constructor Initialization
{
...
//Center is in the initialise method
...
void CMT::initialise(cv::Mat im_gray0, cv::Point2f topleft, cv::Point2f bottomright)
{
    ...
    detector->detect(im_gray0, keypoints);//detecting key points based on BRISK detector type
    ...
    inout_rect(keypoints, topleft, bottomright, selected_keypoints, background_keypoints);//Method to find key points with in the bounding box
    descriptorExtractor->compute(im_gray0, selected_keypoints, selectedFeatures);//Feature extraction based on remembered key points

    if(selected_keypoints.size() == 0)//Key points with in the bounding box
    {
    ...
    descriptorExtractor->compute(im_gray0, background_keypoints, background_features);//Background feature extraction using Background Key points
    ...
    for(unsigned int i = 0; i < selected_keypoints.size(); i++)
        center += selected_keypoints[i].pt;//Find the x and y co-ordinates of each selected key points
    center *= (1.0/selected_keypoints.size());//Find the average center
    CENTER=center;//New line for Biclops
    ...
    for(unsigned int i = 0; i < selected_keypoints.size(); i++)
        springs.push_back(selected_keypoints[i].pt - center);//what is a spring point ?
...
void CMT::processFrame(cv::Mat im_gray)
{
    ...
    std::cout<<"Estimating the frame"<<std::endl;
    estimate(trackedKeypoints, center, scaleEstimate, rotationEstimate, trackedKeypoints2);//Used for estimation of center, scaling and 											    Orientation
